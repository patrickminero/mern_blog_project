        ...
        blogs.map(blog => {
            return(
                <div className="card my-3 blog shadow p-3 mb-5 bg-white rounded " key={blog._id}>
                    ...
                    {this.props.loggedIn ? <Link to={'/allblogs'} className="btn btn-sm btn-info float-right mx-1" onClick={() => {this.props.deleteBlog(blog._id)}}>Delete</Link> : ''}
                    {this.props.loggedIn ? <Link to={'/update' } className="btn btn-sm btn-info float-right mx-1">Update</Link> : ''}
